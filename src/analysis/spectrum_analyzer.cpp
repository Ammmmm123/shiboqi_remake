...
void SpectrumAnalyzer::performFFT()
{
    ...
    // 修正频率分辨率计算：基于实际采样点数和采样率
    // 实际时间跨度 = 采样点数 / 采样率
    // 频率分辨率 = 采样率 / FFT点数（扩展后）
    // 关键：使用原始采样点数的实际时间跨度
    int actualSamples = accumulatedVoltages.size();
    double actualDuration = actualSamples / sampleRate;  // 实际时间跨度（秒）
    double freqResolution = 1.0 / actualDuration;         // 正确的频率分辨率
    
    qDebug() << "FFT参数：实际采样点=" << actualSamples 
             << "，FFT点数=" << n 
             << "，采样率=" << sampleRate 
             << "，时间跨度=" << actualDuration << "s"
             << "，频率分辨率=" << freqResolution << "Hz";

    for (int i = 0; i < halfN; ++i) {
        frequencies[i] = i * freqResolution;
        // 幅度 = |FFT[i]| * 2 / N，并补偿汉宁窗衰减
        // 汉宁窗会使幅度衰减约50%，需要除以窗函数的平均值
        amplitudes[i] = std::abs(fftInput[i]) * 2.0 / (actualSamples * windowCorrection);
    }
    ...
    // 降低刷新率：每N次分析才发射一次信号（深存储模式下减少GUI更新）
